...
public class JNCryptorFactory {
     ...
     * @throws ClassNotFoundException if the version does not correspond to a supported type
     */
    public static JNCryptor getCryptorForCiphertext(byte[] ciphertext)
        ...
        if (ciphertext.length < 1)
            throw new IllegalArgumentException("Ciphertext cannot be zero length.");
     ...
     * @param cryptor the {@link JNCryptor}
     */
     ...
     *
     * @param version the version number. A positive number smaller than 256 (must be
     *                expressible in eight bits).
     * @return the {@link JNCryptor}
     * @throws ClassNotFoundException if no implementation exists for that version
     */
